...
type KsBuilderLister interface {
	...
	List(selector labels.Selector) (ret []*devops.KsBuilder, err error)
...
// List lists all KsBuilders in the indexer.
func (s *ksBuilderLister) List(selector labels.Selector) (ret []*devops.KsBuilder, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*devops.KsBuilder))
	})
	return ret, err
}
...
type KsBuilderNamespaceLister interface {
	...
	Get(name string) (*devops.KsBuilder, error)
...
// List lists all KsBuilders in the indexer for a given namespace.
func (s ksBuilderNamespaceLister) List(selector labels.Selector) (ret []*devops.KsBuilder, err error) {
...
// Get retrieves the KsBuilder from the indexer for a given namespace and name.
func (s ksBuilderNamespaceLister) Get(name string) (*devops.KsBuilder, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(devops.Resource("ksbuilder"), name)
	}
	return obj.(*devops.KsBuilder), nil
}
