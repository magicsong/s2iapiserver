...
type KsBuilderRunLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.KsBuilderRun, err error)
...
// List lists all KsBuilderRuns in the indexer.
func (s *ksBuilderRunLister) List(selector labels.Selector) (ret []*v1alpha1.KsBuilderRun, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.KsBuilderRun))
	})
	return ret, err
}
...
type KsBuilderRunNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.KsBuilderRun, error)
...
// List lists all KsBuilderRuns in the indexer for a given namespace.
func (s ksBuilderRunNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.KsBuilderRun, err error) {
...
// Get retrieves the KsBuilderRun from the indexer for a given namespace and name.
func (s ksBuilderRunNamespaceLister) Get(name string) (*v1alpha1.KsBuilderRun, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("ksbuilderrun"), name)
	}
	return obj.(*v1alpha1.KsBuilderRun), nil
}
