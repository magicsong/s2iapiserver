...
type S2iBuilderLister interface {
	...
	List(selector labels.Selector) (ret []*devops.S2iBuilder, err error)
...
// List lists all S2iBuilders in the indexer.
func (s *s2iBuilderLister) List(selector labels.Selector) (ret []*devops.S2iBuilder, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*devops.S2iBuilder))
	})
	return ret, err
}
...
type S2iBuilderNamespaceLister interface {
	...
	Get(name string) (*devops.S2iBuilder, error)
...
// List lists all S2iBuilders in the indexer for a given namespace.
func (s s2iBuilderNamespaceLister) List(selector labels.Selector) (ret []*devops.S2iBuilder, err error) {
...
// Get retrieves the S2iBuilder from the indexer for a given namespace and name.
func (s s2iBuilderNamespaceLister) Get(name string) (*devops.S2iBuilder, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(devops.Resource("s2ibuilder"), name)
	}
	return obj.(*devops.S2iBuilder), nil
}
